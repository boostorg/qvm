...
boost
    ...
    qvm
        ...
        to_string( A const & a )
            ...
            return '('+
                to_string(q_traits<A>::template r<0>(a))+','+
                to_string(q_traits<A>::template r<1>(a))+','+
                to_string(q_traits<A>::template r<2>(a))+','+
                to_string(q_traits<A>::template r<3>(a))+')';
            }
        ...
        template <class T>
        struct
        deduce_q< qvm_detail::identity_q_<T> >
            {
            typedef quat<T> type;
            };
        ...
        template <class T>
        struct
        deduce_q2< qvm_detail::identity_q_<T>, qvm_detail::identity_q_<T> >
            {
            typedef quat<T> type;
            };
        ...
        template <class V>
        struct
        deduce_q< qvm_detail::rot_q_<V> >
            {
            typedef quat<typename v_traits<V>::scalar_type> type;
            };
        ...
        BOOST_QVM_INLINE_OPERATIONS
        typename enable_if_c<
            is_q<A>::value && is_v<B>::value && v_traits<B>::dim==3,
            typename q_traits<A>::scalar_type>::type
        axis_angle( A const & a, B & b )
            {
            typedef typename q_traits<A>::scalar_type T;
            T a0=q_traits<A>::template r<0>(a);
            T a1=q_traits<A>::template r<1>(a);
            T a2=q_traits<A>::template r<2>(a);
            T a3=q_traits<A>::template r<3>(a);
            if( a0>1 )
                {
                T const mag2=a0*a0+a1*a1+a2*a2+a3*a3;
                if( mag2==s_traits<T>::value(0) )
                    BOOST_THROW_EXCEPTION(zero_magnitude_error());
            ...
            if( T s=sqrt<T>(1-a0*a0) )
                {
                v_traits<B>::template w<0>(b) = a1/s;
                v_traits<B>::template w<1>(b) = a2/s;
                v_traits<B>::template w<2>(b) = a3/s;
                }
            else
                {
                typedef typename v_traits<B>::scalar_type T;
                v_traits<B>::template w<0>(b) = s_traits<T>::value(1);
                v_traits<B>::template w<1>(b) = v_traits<B>::template w<2>(b) = s_traits<T>::value(0);
                }
            return s_traits<T>::value(2) * qvm::acos(a0);
            }
