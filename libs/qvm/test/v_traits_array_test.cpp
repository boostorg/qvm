...
test_ref_cast( T & v, P * ptr )
    ...
    BOOST_QVM_STATIC_ASSERT(v_traits<T>::dim==3);
    BOOST_TEST(v_traits<T>::template r<0>(v)==ptr[0]);
    BOOST_TEST(v_traits<T>::template r<1>(v)==ptr[1]);
    BOOST_TEST(v_traits<T>::template r<2>(v)==ptr[2]);
    BOOST_TEST(&v_traits<T>::template w<0>(v)==&ptr[0]);
    BOOST_TEST(&v_traits<T>::template w<1>(v)==&ptr[1]);
    BOOST_TEST(&v_traits<T>::template w<2>(v)==&ptr[2]);
