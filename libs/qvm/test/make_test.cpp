...
namespace
    ...
    test_matrix()
        ...
        test_qvm::matrix<M2,Rows,Cols> const y=make< test_qvm::matrix<M2,Rows,Cols> >(x);
    ...
    test_vector()
        ...
        test_qvm::vector<V2,Dim> const y=make< test_qvm::vector<V2,Dim> >(x);
        BOOST_QVM_TEST_EQ(x.a,y.a);
        }
    ...
    test_quaternion()
        ...
        normalize(x);
            {
            test_qvm::quaternion<Q2> const y=make< test_qvm::quaternion<Q2> >(x);
            BOOST_QVM_TEST_EQ(x.a,y.a);
            }
            {
            test_qvm::matrix<M1,3,3> const my=make< test_qvm::matrix<M1,3,3> >(x);
            test_qvm::quaternion<Q1> const qy=make< test_qvm::quaternion<Q1> >(my);
            BOOST_QVM_TEST_CLOSE(x.a,qy.a,0.00001f);
            }
            {
            test_qvm::matrix<M1,4,4> const my=make< test_qvm::matrix<M1,4,4> >(x);
